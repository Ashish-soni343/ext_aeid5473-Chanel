...
class ChanelNorwaySpider(scrapy.Spider):   # Starting our spider with a class which contains url to initiate crawling
    name = 'chanel_norway'
    site = 'https://www.chanel.com'

    start_urls = ['https://www.chanel.com/no/']
    frag_url = 'https://www.chanel.com/no/fragrance/women/c/7x1x1/page-'
    frag_m_url = 'https://www.chanel.com/no/fragrance/men/c/7x1x2/page-'
    eyegls_url = 'https://www.chanel.com/no/eyewear/optical/c/2x1x2/page-'
    sungls_url = 'https://www.chanel.com/no/eyewear/sunglasses/c/2x1x1/page-'
    watch_url = 'https://www.chanel.com/no/watches/collection/c/4x2/page-'
    fine_jewellery = 'https://www.chanel.com/no/fine-jewellery/collection/c/3x2/page-'
    face_url = 'https://www.chanel.com/no/makeup/complexion/c/5x1x6/page-'
    eye_url = 'https://www.chanel.com/no/makeup/eyes/c/5x1x4/page-'
    lips_url = 'https://www.chanel.com/no/makeup/lips/c/5x1x1/page-'
    nails_url = 'https://www.chanel.com/no/makeup/nails/c/5x1x7/page-'
    brushes_url = 'https://www.chanel.com/no/makeup/brushes-and-accessories/c/5x1x3/page-'
    skin_care = 'https://www.chanel.com/no/skincare/category/c/6x1/page-'
    skin_line_url = 'https://www.chanel.com/no/skincare/collection/c/6x2/page-'
    frag_bb_url = 'https://www.chanel.com/no/fragrance/bath-and-body/c/7x1x7/page-'
    ...
    def parse(self, response):  # Getting the links from selector and filtering the required urls
        for link in response.css('ul.header__category__links a::attr(href)'):
            a = link.get()
            if a == '/no/eyewear/sunglasses/c/2x1x1/':
                page = self.site + '/no/eyewear/sunglasses/c/2x1x1/'
                next_page = response.follow(url=page, callback=self.sunglass)
                yield next_page

            elif a == '/no/eyewear/optical/c/2x1x2/':
                page1 = self.site + '/no/eyewear/optical/c/2x1x2/'
                next_page1 = response.follow(url=page1, callback=self.eyeglass)
                yield next_page1

            elif a == '/no/fragrance/women/c/7x1x1/':
                page2 = self.site + '/no/fragrance/women/c/7x1x1/'
                yield response.follow(url=page2, callback=self.fragrance)

            elif a == '/no/watches/collection/c/4x2/':
                page3 = self.site + '/no/watches/collection/c/4x2/'
                yield response.follow(url=page3, callback=self.watches)

            elif a == '/no/fine-jewellery/collection/c/3x2/':
                page4 = self.site + '/no/fine-jewellery/collection/c/3x2/'
                yield response.follow(url=page4, callback=self.fine_jwlry)

            elif a == '/no/makeup/complexion/c/5x1x6/':
                page5 = self.site + '/no/makeup/complexion/c/5x1x6/'
                yield response.follow(url=page5, callback=self.mkp_face)

            elif a == '/no/makeup/eyes/c/5x1x4/':
                page6 = self.site + '/no/makeup/eyes/c/5x1x4/'
                yield response.follow(url=page6, callback=self.mkp_eye)

            elif a == '/no/makeup/lips/c/5x1x1/':
                page7 = self.site + '/no/makeup/lips/c/5x1x1/'
                yield response.follow(url=page7, callback=self.mkp_lips)

            elif a == '/no/makeup/nails/c/5x1x7/':
                page8 = self.site + '/no/makeup/nails/c/5x1x7/'
                yield response.follow(url=page8, callback=self.mkp_nails)

            elif a == '/no/makeup/brushes-and-accessories/c/5x1x3/':
                page9 = self.site + '/no/makeup/brushes-and-accessories/c/5x1x3/'
                yield response.follow(url=page9, callback=self.mkp_brushes)

            elif a == '/no/fashion/ready-to-wear/':
                page10 = self.site + '/no/fashion/ready-to-wear/'
                yield response.follow(page10, callback=self.parse_page)

            elif a == '/no/skincare/category/c/6x1/':
                page11 = self.site + '/no/skincare/category/c/6x1/'
                yield response.follow(page11, callback=self.skin)

            elif a == '/no/skincare/collection/c/6x2/':
                page12 = self.site + '/no/skincare/collection/c/6x2/'
                yield response.follow(page12, callback=self.skin_line)

            elif a == '/no/fragrance/men/c/7x1x2/':
                page13 = self.site + '/no/fragrance/men/c/7x1x2/'
                yield response.follow(url=page13, callback=self.frag_m)

            elif a == '/no/fragrance/bath-and-body/c/7x1x7/':
                page14 = self.site + '/no/fragrance/bath-and-body/c/7x1x7/'
