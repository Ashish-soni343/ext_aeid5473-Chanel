...
class ChnlSpider(scrapy.Spider):  # Starting our spider with a class which contains url to initiate crawling
    name = 'chanel'
    site = 'https://www.chanel.com'

    start_urls = ['https://www.chanel.com/us/']
    frag_url = 'https://www.chanel.com/us/fragrance/women/c/7x1x1/page-'
    frag_m_url = 'https://www.chanel.com/us/fragrance/men/c/7x1x2/page-'
    ...
    skin_care = 'https://www.chanel.com/us/skincare/category/c/6x1/page-'
    skin_line_url = 'https://www.chanel.com/us/skincare/collection/c/6x2/page-'
    frag_bb_url = 'https://www.chanel.com/us/fragrance/bath-and-body/c/7x1x7/page-'
    ...
    def parse(self, response):  # Getting the links from selector and filtering the required urls
        for link in response.css('ul.header__category__links a::attr(href)'):
            a = link.get()
            if a == '/us/eyewear/sunglasses/c/2x1x1/':
                page = self.site + '/us/eyewear/sunglasses/c/2x1x1/'
                next_page = response.follow(url=page, callback=self.sunglass)
                yield next_page

            elif a == '/us/eyewear/eyeglasses/c/2x1x2/':
                page1 = self.site + '/us/eyewear/eyeglasses/c/2x1x2/'
                next_page1 = response.follow(url=page1, callback=self.eyeglass)
                yield next_page1

            elif a == '/us/fragrance/women/c/7x1x1/':
                page2 = self.site + '/us/fragrance/women/c/7x1x1/'
                yield response.follow(url=page2, callback=self.fragrance)

            elif a == '/us/watches/collection/c/4x2/':
                page3 = self.site + '/us/watches/collection/c/4x2/'
                yield response.follow(url=page3, callback=self.watches)

            elif a == '/us/fine-jewelry/collection/c/3x2/':
                page4 = self.site + '/us/fine-jewelry/collection/c/3x2/'
                yield response.follow(url=page4, callback=self.fine_jwlry)

            elif a == '/us/makeup/face/c/5x1x6/':
                page5 = self.site + '/us/makeup/face/c/5x1x6/'
                yield response.follow(url=page5, callback=self.mkp_face)

            elif a == '/us/makeup/eyes/c/5x1x4/':
                page6 = self.site + '/us/makeup/eyes/c/5x1x4/'
                yield response.follow(url=page6, callback=self.mkp_eye)

            elif a == '/us/makeup/lips/c/5x1x1/':
                page7 = self.site + '/us/makeup/lips/c/5x1x1/'
                yield response.follow(url=page7, callback=self.mkp_lips)

            elif a == '/us/makeup/nails/c/5x1x7/':
                page8 = self.site + '/us/makeup/nails/c/5x1x7/'
                yield response.follow(url=page8, callback=self.mkp_nails)

            elif a == '/us/makeup/brushes-and-accessories/c/5x1x3/':
                page9 = self.site + '/us/makeup/brushes-and-accessories/c/5x1x3/'
                yield response.follow(url=page9, callback=self.mkp_brushes)

            elif a == '/us/fashion/ready-to-wear/':
                page10 = self.site + '/us/fashion/ready-to-wear/'
                yield response.follow(page10, callback=self.parse_page)

            elif a == '/us/skincare/category/c/6x1/':
                page11 = self.site + '/us/skincare/category/c/6x1/'
                yield response.follow(page11, callback=self.skin)

            elif a == '/us/skincare/collection/c/6x2/':
                page12 = self.site + '/us/skincare/collection/c/6x2/'
                yield response.follow(page12, callback=self.skin_line)

            elif a == '/us/fragrance/men/c/7x1x2/':
                page13 = self.site + '/us/fragrance/men/c/7x1x2/'
                yield response.follow(url=page13, callback=self.frag_m)

            elif a == '/us/fragrance/bath-and-body/c/7x1x7/':
                page14 = self.site + '/us/fragrance/bath-and-body/c/7x1x7/'
