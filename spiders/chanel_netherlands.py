...
class ChanelNetherlandsSpider(scrapy.Spider):    # Starting our spider with a class which contains url to initiate crawling
    name = 'chanel_netherlands'
    site = 'https://www.chanel.com'

    start_urls = ['https://www.chanel.com/nl/']
    frag_url = 'https://www.chanel.com/nl/parfums/dames/c/7x1x1/page-'
    frag_m_url = 'https://www.chanel.com/nl/parfums/heren/c/7x1x2/page-'
    eyegls_url = 'https://www.chanel.com/nl/brillen/brillen/c/2x1x2/page-'
    sungls_url = 'https://www.chanel.com/nl/brillen/zonnebrillen/c/2x1x1/page-'
    watch_url = 'https://www.chanel.com/nl/horloges/collectie/c/4x2/page-'
    fine_jewellery = 'https://www.chanel.com/nl/sieraden/collectie/c/3x2/page-'
    face_url = 'https://www.chanel.com/nl/make-up/teint/c/5x1x6/page-'
    eye_url = 'https://www.chanel.com/nl/make-up/ogen/c/5x1x4/page-'
    lips_url = 'https://www.chanel.com/nl/make-up/lippen/c/5x1x1/page-'
    nails_url = 'https://www.chanel.com/nl/make-up/nagels/c/5x1x7/page-'
    brushes_url = 'https://www.chanel.com/nl/make-up/penselen-en-accessoires/c/5x1x3/page-'
    skin_care = 'https://www.chanel.com/nl/huidverzorging/categorie/c/6x1/page-'
    skin_line_url = 'https://www.chanel.com/nl/huidverzorging/collectie/c/6x2/page-'
    frag_bb_url = 'https://www.chanel.com/nl/parfums/bad-en-lichaam/c/7x1x7/page-'
    ...
    def parse(self, response):  # Getting the links from selector and filtering the required urls
        for link in response.css('ul.header__category__links a::attr(href)'):
            a = link.get()
            if a == '/nl/brillen/zonnebrillen/c/2x1x1/':
                page = self.site + '/nl/brillen/zonnebrillen/c/2x1x1/'
                next_page = response.follow(url=page, callback=self.sunglass)
                yield next_page

            elif a == '/nl/brillen/brillen/c/2x1x2/':
                page1 = self.site + '/nl/brillen/brillen/c/2x1x2/'
                next_page1 = response.follow(url=page1, callback=self.eyeglass)
                yield next_page1

            elif a == '/nl/parfums/dames/c/7x1x1/':
                page2 = self.site + '/nl/parfums/dames/c/7x1x1/'
                yield response.follow(url=page2, callback=self.fragrance)

            elif a == '/nl/horloges/collectie/c/4x2/':
                page3 = self.site + '/nl/horloges/collectie/c/4x2/'
                yield response.follow(url=page3, callback=self.watches)

            elif a == '/nl/sieraden/collectie/c/3x2/':
                page4 = self.site + '/nl/sieraden/collectie/c/3x2/'
                yield response.follow(url=page4, callback=self.fine_jwlry)

            elif a == '/nl/make-up/teint/c/5x1x6/':
                page5 = self.site + '/nl/make-up/teint/c/5x1x6/'
                yield response.follow(url=page5, callback=self.mkp_face)

            elif a == '/nl/make-up/ogen/c/5x1x4/':
                page6 = self.site + '/nl/make-up/ogen/c/5x1x4/'
                yield response.follow(url=page6, callback=self.mkp_eye)

            elif a == '/nl/make-up/lippen/c/5x1x1/':
                page7 = self.site + '/nl/make-up/lippen/c/5x1x1/'
                yield response.follow(url=page7, callback=self.mkp_lips)

            elif a == '/nl/make-up/nagels/c/5x1x7/':
                page8 = self.site + '/nl/make-up/nagels/c/5x1x7/'
                yield response.follow(url=page8, callback=self.mkp_nails)

            elif a == '/nl/make-up/penselen-en-accessoires/c/5x1x3/':
                page9 = self.site + '/nl/make-up/penselen-en-accessoires/c/5x1x3/'
                yield response.follow(url=page9, callback=self.mkp_brushes)

            elif a == '/nl/mode/pret-a-porter/':
                page10 = self.site + '/nl/mode/pret-a-porter/'
                yield response.follow(page10, callback=self.parse_page)

            elif a == '/nl/huidverzorging/categorie/c/6x1/':
                page11 = self.site + '/nl/huidverzorging/categorie/c/6x1/'
                yield response.follow(page11, callback=self.skin)

            elif a == '/nl/huidverzorging/collectie/c/6x2/':
                page12 = self.site + '/nl/huidverzorging/collectie/c/6x2/'
                yield response.follow(page12, callback=self.skin_line)

            elif a == '/nl/parfums/heren/c/7x1x2/':
                page13 = self.site + '/nl/parfums/heren/c/7x1x2/'
                yield response.follow(url=page13, callback=self.frag_m)

            elif a == '/nl/parfums/bad-en-lichaam/c/7x1x7/':
                page14 = self.site + '/nl/parfums/bad-en-lichaam/c/7x1x7/'
