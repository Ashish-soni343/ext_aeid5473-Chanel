...
class ChanelFinlandSpider(scrapy.Spider):    # Starting our spider with a class which contains url to initiate crawling
    name = 'chanel_finland'
    site = 'https://www.chanel.com'

    start_urls = ['https://www.chanel.com/fi']
    frag_url = 'https://www.chanel.com/fi/fragrance/women/c/7x1x1/page-'
    frag_m_url = 'https://www.chanel.com/fi/fragrance/men/c/7x1x2/page-'
    eyegls_url = 'https://www.chanel.com/fi/eyewear/optical/c/2x1x2/page-'
    sungls_url = 'https://www.chanel.com/fi/eyewear/sunglasses/c/2x1x1/page-'
    watch_url = 'https://www.chanel.com/fi/watches/collection/c/4x2/page-'
    fine_jewellery = 'https://www.chanel.com/fi/fine-jewellery/collection/c/3x2/page-'
    face_url = 'https://www.chanel.com/fi/makeup/complexion/c/5x1x6/page-'
    eye_url = 'https://www.chanel.com/fi/makeup/eyes/c/5x1x4/page-'
    lips_url = 'https://www.chanel.com/fi/makeup/lips/c/5x1x1/page-'
    nails_url = 'https://www.chanel.com/fi/makeup/nails/c/5x1x7/page-'
    brushes_url = 'https://www.chanel.com/fi/makeup/brushes-and-accessories/c/5x1x3/page-'
    skin_care = 'https://www.chanel.com/fi/skincare/category/c/6x1/page-'
    skin_line_url = 'https://www.chanel.com/fi/skincare/collection/c/6x2/page-'
    frag_bb_url = 'https://www.chanel.com/fi/fragrance/bath-and-body/c/7x1x7/page-'
    ...
    def parse(self, response):  # Getting the links from selector and filtering the required urls
        for link in response.css('ul.header__category__links a::attr(href)'):
            a = link.get()
            if a == '/fi/eyewear/sunglasses/c/2x1x1/':
                page = self.site + '/fi/eyewear/sunglasses/c/2x1x1/'
                next_page = response.follow(url=page, callback=self.sunglass)
                yield next_page

            elif a == '/fi/eyewear/optical/c/2x1x2/':
                page1 = self.site + '/fi/eyewear/optical/c/2x1x2/'
                next_page1 = response.follow(url=page1, callback=self.eyeglass)
                yield next_page1

            elif a == '/fi/fragrance/women/c/7x1x1/':
                page2 = self.site + '/fi/fragrance/women/c/7x1x1/'
                yield response.follow(url=page2, callback=self.fragrance)

            elif a == '/fi/watches/collection/c/4x2/':
                page3 = self.site + '/fi/watches/collection/c/4x2/'
                yield response.follow(url=page3, callback=self.watches)

            elif a == '/fi/fine-jewellery/collection/c/3x2/':
                page4 = self.site + '/fi/fine-jewellery/collection/c/3x2/'
                yield response.follow(url=page4, callback=self.fine_jwlry)

            elif a == '/fi/makeup/complexion/c/5x1x6/':
                page5 = self.site + '/fi/makeup/complexion/c/5x1x6/'
                yield response.follow(url=page5, callback=self.mkp_face)

            elif a == '/fi/makeup/eyes/c/5x1x4/':
                page6 = self.site + '/fi/makeup/eyes/c/5x1x4/'
                yield response.follow(url=page6, callback=self.mkp_eye)

            elif a == '/fi/makeup/lips/c/5x1x1/':
                page7 = self.site + '/fi/makeup/lips/c/5x1x1/'
                yield response.follow(url=page7, callback=self.mkp_lips)

            elif a == '/fi/makeup/nails/c/5x1x7/':
                page8 = self.site + '/fi/makeup/nails/c/5x1x7/'
                yield response.follow(url=page8, callback=self.mkp_nails)

            elif a == '/fi/makeup/brushes-and-accessories/c/5x1x3/':
                page9 = self.site + '/fi/makeup/brushes-and-accessories/c/5x1x3/'
                yield response.follow(url=page9, callback=self.mkp_brushes)

            elif a == '/fi/fashion/ready-to-wear/':
                page10 = self.site + '/fi/fashion/ready-to-wear/'
                yield response.follow(page10, callback=self.parse_page)

            elif a == '/fi/skincare/category/c/6x1/':
                page11 = self.site + '/fi/skincare/category/c/6x1/'
                yield response.follow(page11, callback=self.skin)

            elif a == '/fi/skincare/collection/c/6x2/':
                page12 = self.site + '/fi/skincare/collection/c/6x2/'
                yield response.follow(page12, callback=self.skin_line)

            elif a == '/fi/fragrance/men/c/7x1x2/':
                page13 = self.site + '/fi/fragrance/men/c/7x1x2/'
                yield response.follow(url=page13, callback=self.frag_m)

            elif a == '/fi/fragrance/bath-and-body/c/7x1x7/':
                page14 = self.site + '/fi/fragrance/bath-and-body/c/7x1x7/'
