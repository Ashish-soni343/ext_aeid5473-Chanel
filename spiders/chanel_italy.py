...
class ChanelItalySpider(scrapy.Spider):  # Starting our spider with a class which contains url to initiate crawling
    name = 'chanel_italy'
    site = 'https://www.chanel.com'

    start_urls = ['https://www.chanel.com/it/']
    frag_url = 'https://www.chanel.com/it/profumi/donna/c/7x1x1/page-'
    frag_m_url = 'https://www.chanel.com/it/profumi/uomo/c/7x1x2/page-'
    eyegls_url = 'https://www.chanel.com/it/occhiali/occhiali-da-vista/c/2x1x2/page-'
    sungls_url = 'https://www.chanel.com/it/occhiali/occhiali-da-sole/c/2x1x1/page-'
    watch_url = 'https://www.chanel.com/it/orologi/collezione/c/4x2/page-'
    fine_jewellery = 'https://www.chanel.com/it/gioielleria/collezione/c/3x2/page-'
    face_url = 'https://www.chanel.com/it/make-up/colorito/c/5x1x6/page-'
    eye_url = 'https://www.chanel.com/it/make-up/occhi/c/5x1x4/page-'
    lips_url = 'https://www.chanel.com/it/make-up/labbra/c/5x1x1/page-'
    nails_url = 'https://www.chanel.com/it/make-up/unghie/c/5x1x7/page-'
    brushes_url = 'https://www.chanel.com/it/make-up/pennelli-e-accessori/c/5x1x3/page-'
    skin_care = 'https://www.chanel.com/it/skincare/categoria/c/6x1/page-'
    skin_line_url = 'https://www.chanel.com/it/skincare/collezione/c/6x2/page-'
    frag_bb_url = 'https://www.chanel.com/it/profumi/bagno-e-corpo/c/7x1x7/page-'
    ...
    def parse(self, response):  # Getting the links from selector and filtering the required urls
        for link in response.css('ul.header__category__links a::attr(href)'):
            a = link.get()
            if a == '/it/occhiali/occhiali-da-sole/c/2x1x1/':
                page = self.site + '/it/occhiali/occhiali-da-sole/c/2x1x1/'
                next_page = response.follow(url=page, callback=self.sunglass)
                yield next_page

            elif a == '/it/occhiali/occhiali-da-vista/c/2x1x2/':
                page1 = self.site + '/it/occhiali/occhiali-da-vista/c/2x1x2/'
                next_page1 = response.follow(url=page1, callback=self.eyeglass)
                yield next_page1

            elif a == '/it/profumi/donna/c/7x1x1/':
                page2 = self.site + '/it/profumi/donna/c/7x1x1/'
                yield response.follow(url=page2, callback=self.fragrance)

            elif a == '/it/orologi/collezione/c/4x2/':
                page3 = self.site + '/it/orologi/collezione/c/4x2/'
                yield response.follow(url=page3, callback=self.watches)

            elif a == '/it/gioielleria/collezione/c/3x2/':
                page4 = self.site + '/it/gioielleria/collezione/c/3x2/'
                yield response.follow(url=page4, callback=self.fine_jwlry)

            elif a == '/it/make-up/colorito/c/5x1x6/':
                page5 = self.site + '/it/make-up/colorito/c/5x1x6/'
                yield response.follow(url=page5, callback=self.mkp_face)

            elif a == '/it/make-up/occhi/c/5x1x4/':
                page6 = self.site + '/it/make-up/occhi/c/5x1x4/'
                yield response.follow(url=page6, callback=self.mkp_eye)

            elif a == '/it/make-up/labbra/c/5x1x1/':
                page7 = self.site + '/it/make-up/labbra/c/5x1x1/'
                yield response.follow(url=page7, callback=self.mkp_lips)

            elif a == '/it/make-up/unghie/c/5x1x7/':
                page8 = self.site + '/it/make-up/unghie/c/5x1x7/'
                yield response.follow(url=page8, callback=self.mkp_nails)

            elif a == '/it/make-up/pennelli-e-accessori/c/5x1x3/':
                page9 = self.site + '/it/make-up/pennelli-e-accessori/c/5x1x3/'
                yield response.follow(url=page9, callback=self.mkp_brushes)

            elif a == '/it/moda/pret-a-porter/':
                page10 = self.site + '/it/moda/pret-a-porter/'
                yield response.follow(page10, callback=self.parse_page)

            elif a == '/it/skincare/categoria/c/6x1/':
                page11 = self.site + '/it/skincare/categoria/c/6x1/'
                yield response.follow(page11, callback=self.skin)

            elif a == '/it/skincare/collezione/c/6x2/':
                page12 = self.site + '/it/skincare/collezione/c/6x2/'
                yield response.follow(page12, callback=self.skin_line)

            elif a == '/it/profumi/uomo/c/7x1x2/':
                page13 = self.site + '/it/profumi/uomo/c/7x1x2/'
                yield response.follow(url=page13, callback=self.frag_m)

            elif a == '/it/profumi/bagno-e-corpo/c/7x1x7/':
                page14 = self.site + '/it/profumi/bagno-e-corpo/c/7x1x7/'
