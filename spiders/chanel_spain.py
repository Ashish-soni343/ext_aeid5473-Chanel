...
class ChanelSpainSpider(scrapy.Spider):  # Starting our spider with a class which contains url to initiate crawling
    name = 'chanel_spain'
    site = 'https://www.chanel.com'

    start_urls = ['https://www.chanel.com/es/']
    frag_url = 'https://www.chanel.com/es/perfumes/femeninos/c/7x1x1/page-'
    frag_m_url = 'https://www.chanel.com/es/perfumes/masculinos/c/7x1x2/page-'
    eyegls_url = 'https://www.chanel.com/es/gafas/gafas-para-graduar/c/2x1x2/page-'
    sungls_url = 'https://www.chanel.com/es/gafas/gafas-de-sol/c/2x1x1/page-'
    watch_url = 'https://www.chanel.com/es/relojes/coleccion/c/4x2/page-'
    fine_jewellery = 'https://www.chanel.com/es/joyeria/coleccion/c/3x2/page-'
    face_url = 'https://www.chanel.com/es/maquillaje/tez/c/5x1x6/page-'
    eye_url = 'https://www.chanel.com/es/maquillaje/ojos/c/5x1x4/page-'
    lips_url = 'https://www.chanel.com/es/maquillaje/labios/c/5x1x1/page-'
    nails_url = 'https://www.chanel.com/es/maquillaje/unas/c/5x1x7/page-'
    brushes_url = 'https://www.chanel.com/es/maquillaje/pinceles-y-accesorios/c/5x1x3/page-'
    skin_care = 'https://www.chanel.com/es/tratamiento/categoria/c/6x1/page-'
    skin_line_url = 'https://www.chanel.com/es/tratamiento/coleccion/c/6x2/page-'
    frag_bb_url = 'https://www.chanel.com/es/perfumes/bano-y-cuerpo/c/7x1x7/page-'
    ...
    def parse(self, response):  # Getting the links from selector and filtering the required urls
        for link in response.css('ul.header__category__links a::attr(href)'):
            a = link.get()
            if a == '/es/gafas/gafas-de-sol/c/2x1x1/':
                page = self.site + '/es/gafas/gafas-de-sol/c/2x1x1/'
                next_page = response.follow(url=page, callback=self.sunglass)
                yield next_page

            elif a == '/es/gafas/gafas-para-graduar/c/2x1x2/':
                page1 = self.site + '/es/gafas/gafas-para-graduar/c/2x1x2/'
                next_page1 = response.follow(url=page1, callback=self.eyeglass)
                yield next_page1

            elif a == '/es/perfumes/femeninos/c/7x1x1/':
                page2 = self.site + '/es/perfumes/femeninos/c/7x1x1/'
                yield response.follow(url=page2, callback=self.fragrance)

            elif a == '/es/relojes/coleccion/c/4x2/':
                page3 = self.site + '/es/relojes/coleccion/c/4x2/'
                yield response.follow(url=page3, callback=self.watches)

            elif a == '/es/joyeria/coleccion/c/3x2/':
                page4 = self.site + '/es/joyeria/coleccion/c/3x2/'
                yield response.follow(url=page4, callback=self.fine_jwlry)

            elif a == '/es/maquillaje/tez/c/5x1x6/':
                page5 = self.site + '/es/maquillaje/tez/c/5x1x6/'
                yield response.follow(url=page5, callback=self.mkp_face)

            elif a == '/es/maquillaje/ojos/c/5x1x4/':
                page6 = self.site + '/es/maquillaje/ojos/c/5x1x4/'
                yield response.follow(url=page6, callback=self.mkp_eye)

            elif a == '/es/maquillaje/labios/c/5x1x1/':
                page7 = self.site + '/es/maquillaje/labios/c/5x1x1/'
                yield response.follow(url=page7, callback=self.mkp_lips)

            elif a == '/es/maquillaje/unas/c/5x1x7/':
                page8 = self.site + '/es/maquillaje/unas/c/5x1x7/'
                yield response.follow(url=page8, callback=self.mkp_nails)

            elif a == '/es/maquillaje/pinceles-y-accesorios/c/5x1x3/':
                page9 = self.site + '/es/maquillaje/pinceles-y-accesorios/c/5x1x3/'
                yield response.follow(url=page9, callback=self.mkp_brushes)

            elif a == '/es/moda/pret-a-porter/':
                page10 = self.site + '/es/moda/pret-a-porter/'
                yield response.follow(page10, callback=self.parse_page)

            elif a == '/es/tratamiento/categoria/c/6x1/':
                page11 = self.site + '/es/tratamiento/categoria/c/6x1/'
                yield response.follow(page11, callback=self.skin)

            elif a == '/es/tratamiento/coleccion/c/6x2/':
                page12 = self.site + '/es/tratamiento/coleccion/c/6x2/'
                yield response.follow(page12, callback=self.skin_line)

            elif a == '/es/perfumes/masculinos/c/7x1x2/':
                page13 = self.site + '/es/perfumes/masculinos/c/7x1x2/'
                yield response.follow(url=page13, callback=self.frag_m)

            elif a == '/es/perfumes/bano-y-cuerpo/c/7x1x7/':
                page14 = self.site + '/es/perfumes/bano-y-cuerpo/c/7x1x7/'
