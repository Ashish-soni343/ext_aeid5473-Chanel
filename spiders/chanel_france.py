...
class ChanelFranceSpider(scrapy.Spider):  # Starting our spider with a class which contains url to initiate crawling
    name = 'chanel_france'
    site = 'https://www.chanel.com'

    start_urls = ['https://www.chanel.com/fr/']
    frag_url = 'https://www.chanel.com/fr/parfums/femmes/c/7x1x1/page-'
    frag_m_url = 'https://www.chanel.com/fr/parfums/hommes/c/7x1x2/page-'
    eyegls_url = 'https://www.chanel.com/fr/lunettes/lunettes-optiques/c/2x1x2/page-'
    sungls_url = 'https://www.chanel.com/fr/lunettes/lunettes-de-soleil/c/2x1x1/page-'
    watch_url = 'https://www.chanel.com/fr/horlogerie/collection/c/4x2/page-'
    fine_jewellery = 'https://www.chanel.com/fr/joaillerie/collection/c/3x2/page-'
    face_url = 'https://www.chanel.com/fr/maquillage/teint/c/5x1x6/page-'
    eye_url = 'https://www.chanel.com/fr/maquillage/yeux/c/5x1x4/page-'
    lips_url = 'https://www.chanel.com/fr/maquillage/levres/c/5x1x1/page-'
    nails_url = 'https://www.chanel.com/fr/maquillage/ongles/c/5x1x7/page-'
    brushes_url = 'https://www.chanel.com/fr/maquillage/pinceaux-et-accessoires/c/5x1x3/page-'
    skin_care = 'https://www.chanel.com/fr/soins/categorie/c/6x1/page-'
    skin_line_url = 'https://www.chanel.com/fr/soins/collection/c/6x2/page-'
    frag_bb_url = 'https://www.chanel.com/fr/parfums/bain-et-corps/c/7x1x7/page-'
    ...
    def parse(self, response):  # Getting the links from selector and filtering the required urls
        for link in response.css('ul.header__category__links a::attr(href)'):
            a = link.get()
            if a == '/fr/lunettes/lunettes-de-soleil/c/2x1x1/':
                page = self.site + '/fr/lunettes/lunettes-de-soleil/c/2x1x1/'
                next_page = response.follow(url=page, callback=self.sunglass)
                yield next_page

            elif a == '/fr/lunettes/lunettes-optiques/c/2x1x2/':
                page1 = self.site + '/fr/lunettes/lunettes-optiques/c/2x1x2/'
                next_page1 = response.follow(url=page1, callback=self.eyeglass)
                yield next_page1

            elif a == '/fr/parfums/femmes/c/7x1x1/':
                page2 = self.site + '/fr/parfums/femmes/c/7x1x1/'
                yield response.follow(url=page2, callback=self.fragrance)

            elif a == '/fr/horlogerie/collection/c/4x2/':
                page3 = self.site + '/fr/horlogerie/collection/c/4x2/'
                yield response.follow(url=page3, callback=self.watches)

            elif a == '/fr/joaillerie/collection/c/3x2/':
                page4 = self.site + '/fr/joaillerie/collection/c/3x2/'
                yield response.follow(url=page4, callback=self.fine_jwlry)

            elif a == '/fr/maquillage/teint/c/5x1x6/':
                page5 = self.site + '/fr/maquillage/teint/c/5x1x6/'
                yield response.follow(url=page5, callback=self.mkp_face)

            elif a == '/fr/maquillage/yeux/c/5x1x4/':
                page6 = self.site + '/fr/maquillage/yeux/c/5x1x4/'
                yield response.follow(url=page6, callback=self.mkp_eye)

            elif a == '/fr/maquillage/levres/c/5x1x1/':
                page7 = self.site + '/fr/maquillage/levres/c/5x1x1/'
                yield response.follow(url=page7, callback=self.mkp_lips)

            elif a == '/fr/maquillage/ongles/c/5x1x7/':
                page8 = self.site + '/fr/maquillage/ongles/c/5x1x7/'
                yield response.follow(url=page8, callback=self.mkp_nails)

            elif a == '/fr/maquillage/pinceaux-et-accessoires/c/5x1x3/':
                page9 = self.site + '/fr/maquillage/pinceaux-et-accessoires/c/5x1x3/'
                yield response.follow(url=page9, callback=self.mkp_brushes)

            elif a == '/fr/mode/pret-a-porter/':
                page10 = self.site + '/fr/mode/pret-a-porter/'
                yield response.follow(page10, callback=self.parse_page)

            elif a == '/fr/soins/categorie/c/6x1/':
                page11 = self.site + '/fr/soins/categorie/c/6x1/'
                yield response.follow(page11, callback=self.skin)

            elif a == '/fr/soins/collection/c/6x2/':
                page12 = self.site + '/fr/soins/collection/c/6x2/'
                yield response.follow(page12, callback=self.skin_line)

            elif a == '/fr/parfums/hommes/c/7x1x2/':
                page13 = self.site + '/fr/parfums/hommes/c/7x1x2/'
                yield response.follow(url=page13, callback=self.frag_m)

            elif a == '/fr/parfums/bain-et-corps/c/7x1x7/':
                page14 = self.site + '/fr/parfums/bain-et-corps/c/7x1x7/'
